 ...
 */
 ...
 zoni_error_t zoni_memory_read32(zoni_memory_t* memory, u32 address, u32* value) {
    ...
    //zoni_error_t zoni_memory_read32(zoni_memory_t* memory, u32 address, u32* value) {
     ...
     // 4️⃣ Default
     *value = 0;
     zoni_log(ZONI_LOG_WARNING, "Invalid read32 at address 0x%08X (phys 0x%08X)", address, addr_phys);
     return ZONI_ERROR_INVALID_PARAMETER;
 }
 ...
 zoni_error_t zoni_memory_write32(zoni_memory_t* memory, u32 address, u32 value) {
     ...
     zoni_log(ZONI_LOG_DEBUG, "MEM WRITE32 ignored (BIOS) [0x%08X] <- 0x%08X", addr_phys, value);
     return ZONI_SUCCESS;
 }
