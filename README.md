...
**Still open**: the main PS1 Display area is dirty — grey vertical bands replace columns that are present and correct in VRAM. Since the CPU-side VRAM model holds the full frame while the unified GL texture does not, the remaining defect is in the upload-to-texture / scanout path, not in decoding.
...
**FMV update (July 26 2026)**: the intro movie now decodes and plays — a clean frame is visible in VRAM (top screenshot). Two DMA bugs were responsible for the previous "black/blob FMV": lost completion interrupts (the IRQ3 line was never deasserted on a DICR-only acknowledge, so after the first one every completion was swallowed and the player's frame queue froze after 49 of 1905 frames), and block transfers on the GPU channel reading guest RAM lazily across slices, which duplicated two staging buffers across all twenty columns of each frame. What remains is a display-side defect: the main PS1 Display area shows grey vertical bands where VRAM holds correct pixels, so the gap is now between VRAM and the unified GL texture / scanout, not in decoding.

Outstanding: the FMV display area is still striped (see above). GPU mask-bit handling and VRAM readback have known gaps affecting some rendering techniques (see `GPU_GAP_ANALYSIS_2026-07-15.md`). SPU audio sync has minor timing issues. Overall stability is still fragile — this is early confirmation a real commercial disc *can* reach its menu/engine, not a claim of solid end-to-end play.
