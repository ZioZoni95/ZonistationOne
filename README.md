...
BIOS boots to the interactive menu, and real commercial discs now boot into gameplay — `Ace Combat 2 (Europe)` reaches the game itself after a CDROM sector-buffer fix (the drive's Request Register was resetting the read pointer on the wrong arm/disarm transition, corrupting any game that re-reads a sector's header before its data). GPU, DMA, timers, CDROM, SIO, GTE, I-Cache, SPU, and a from-scratch MDEC (ported from DuckStation's reference algorithms) are all implemented. The renderer has a full ImGui IDE-style debug interface with CPU disassembler, breakpoint manager, execution tracer, and per-component log windows — including a BIOS/game TTY log with real `printf`-style argument substitution.

Outstanding: GPU mask-bit handling and VRAM readback have known gaps affecting some rendering techniques (see `GPU_GAP_ANALYSIS_2026-07-15.md`). Timer sync-modes are decoded but not yet gating counters. SPU audio sync has minor timing issues. MDEC is implemented but not yet exercised against a real FMV-using game.
