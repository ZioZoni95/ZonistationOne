...
uint32_t vram_load32(Vram* vram, uint32_t offset) {
    if (offset % 4 != 0) {
         LOG_VRAM_WARN("VRAM Load32 unaligned: offset 0x%x", offset);
     ...
     if (is_out_of_bounds(offset, 4)) {
        LOG_VRAM_ERROR("VRAM Load32 out of bounds: offset 0x%x", offset);
...
void vram_store32(Vram* vram, uint32_t offset, uint32_t value) {
     if (offset % 4 != 0) {
         LOG_VRAM_WARN("VRAM Store32 unaligned: offset 0x%x", offset);
        // Proceeding, but acknowledging potential issue.
    }
     if (is_out_of_bounds(offset, 4)) {
        LOG_VRAM_ERROR("VRAM Store32 out of bounds: offset 0x%x", offset);
...
uint16_t vram_load16(Vram* vram, uint32_t offset) {
     if (offset % 2 != 0) {
         LOG_VRAM_WARN("VRAM Load16 unaligned: offset 0x%x", offset);
        // Proceeding, but this is usually an error for pixel access.
     }
     if (is_out_of_bounds(offset, 2)) {
        LOG_VRAM_ERROR("VRAM Load16 out of bounds: offset 0x%x", offset);
        return 0;
    }
...
void vram_store16(Vram* vram, uint32_t offset, uint16_t value) {
     if (offset % 2 != 0) {
         LOG_VRAM_WARN("VRAM Store16 unaligned: offset 0x%x", offset);
        // Proceeding, but this is usually an error for pixel access.
    }
    if (is_out_of_bounds(offset, 2)) {
        LOG_VRAM_ERROR("VRAM Store16 out of bounds: offset 0x%x", offset);
        return;
    }
    vram->data[offset + 0] = (uint8_t)(value & 0xFF);
    vram->data[offset + 1] = (uint8_t)((value >> 8) & 0xFF);
...
uint8_t vram_load8(Vram* vram, uint32_t offset) {
    if (is_out_of_bounds(offset, 1)) {
        LOG_VRAM_ERROR("VRAM Load8 out of bounds: offset 0x%x", offset);
        return 0;
    }
...
void vram_store8(Vram* vram, uint32_t offset, uint8_t value) {
    if (is_out_of_bounds(offset, 1)) {
        LOG_VRAM_ERROR("VRAM Store8 out of bounds: offset 0x%x", offset);
        return;
    }
