/*
 * CDROM Audio: XA-ADPCM decoder, CDDA processing, AudioFifo, SDL output.
 * XA decoder ported from DuckStation (MIT licence).
 */
 ...
 * XA-ADPCM Decoder
 *
 * Ported from DuckStation cdrom.cpp:DecodeXAADPCMChunks / ResampleXAADPCM.
 * Reference: PSX-SPX §7.3 CD-ROM XA-ADPCM Audio.
 * ========================================================================= */
...
/*
 * Decode one 128-byte chunk (18 per sector).
 * stereo: pairs of blocks (L/R interleaved in output).
 * bits8 : 8-bit ADPCM (4 blocks), else 4-bit (8 blocks).
 * xa_prev[ch][0/1]: IIR history, updated in-place.
 * out_samples: [WORDS*NUM_BLOCKS] or [WORDS*NUM_BLOCKS*2] for stereo.
 */
static void decode_xa_chunk(const uint8_t *chunk, bool stereo, bool bits8,
    ...
    for (int block = 0; block < num_blocks; block++) {
        ...
        int     ch   = stereo ? (block & 1) : 0;  /* even=L, odd=R for stereo */

        /* output pointer: stereo interleaves L/R */
        int16_t *out;
        int      step;
        if (stereo) {
        ...
        for (int w = 0; w < words_per_blk; w++) {
            ...
            out[w * step] = clamp16(s);
        }
    }
}

/* Zigzag resampler tables: 7 × 29 coefficients (37800→44100 Hz) */
...
/* 18900 Hz resampler tables: 7 × 25 coefficients */
...
static void resample_xa_37800(XaAdpcmState *xa, AudioFifo *fifo,
                               const int16_t *frames, uint32_t num_frames,
                               bool stereo) {
    uint8_t p       = xa->ring_p;
...
void cdrom_audio_decode_xa(XaAdpcmState *xa, AudioFifo *fifo,
                            ...
                            bool muted) {
    /* Drop sectors if FIFO getting full (low watermark = 2048 frames) */
    ...
    /* Max buffer: 18 chunks × largest case (8-bit stereo: 4×28×4 = 448 samp/chunk, /2 = 224 frames)
       4-bit mono: 8×28×8 = 1792... use 18×1792 = 32256 worst case */
    static int16_t sample_buf[18 * 8 * 28 * 8];

    /* IIR state per channel */
    int32_t prev[2][2];
    ...
    if (rate_18900)
        resample_xa_18900(xa, fifo, sample_buf, total_frames, stereo);
    else
        resample_xa_37800(xa, fifo, sample_buf, total_frames, stereo);
}
...
void cdrom_audio_process_cdda(AudioFifo *fifo, const uint8_t *raw_sector, bool muted) {
    /* CDDA: 588 stereo samples × 4 bytes (no header) */
    for (int i = 0; i < 588; i++) {
...
static void sdl_audio_callback(void *userdata, uint8_t *stream, int len) {
    ...
    int samples  = len / sizeof(int16_t);  /* total int16 values (L+R) */
