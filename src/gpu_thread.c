    ...
    while (!state->shutdown_requested) {
        ...
        while (read_ptr != write_ptr) {
            ...
            switch (cmd->type) {
                ...
                case GPU_CMD_SYNC: {
                    // Synchronization point - signal CPU thread
...
bool gpu_thread_init(GpuThreadState* state, struct GPU* gpu, bool use_threading) {
    ...
    // Allocate command FIFO
    state->command_fifo = (uint8_t*)malloc(state->fifo_size);
...
void* gpu_thread_alloc_command(GpuThreadState* state, uint32_t size) {
    ...
    if (size > free_space) {
        // FIFO full - sync with GPU to free space
        LOG_GPU_WARN("GPU command FIFO full, syncing with GPU thread");
        gpu_thread_sync(state, true);
        
        // Recheck space
        free_space = get_fifo_free_space(state);
        if (size > free_space) {
            LOG_GPU_ERROR("GPU command FIFO still full after sync (need %u, have %u)",
                         size, free_space);
            return NULL;
        }
    }
    ...
    // Handle wrap-around
    if (write_ptr + size > state->fifo_size) {
        // TODO: Handle wrap-around properly
        // For now, sync and reset to beginning
        gpu_thread_sync(state, true);
        atomic_store_u32(&state->write_ptr, 0);
        atomic_store_u32(&state->read_ptr, 0);
        write_ptr = 0;
...
void gpu_thread_submit_command(GpuThreadState* state, GpuCommand* cmd) {
    ...
    uint32_t write_ptr = atomic_load_u32(&state->write_ptr);
    uint32_t new_write_ptr = wrap_fifo_ptr(write_ptr + cmd->size, state->fifo_size);
    atomic_store_u32(&state->write_ptr, new_write_ptr);
    
    // Increment pending command counter
