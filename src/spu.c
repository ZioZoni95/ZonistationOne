...
void spu_init(Spu* spu) {
    ...
    LOG_INTERCONNECT_INFO("SPU: initialized (MMIO %08x-%08x)", SPU_START, SPU_END);
}
...
uint16_t spu_read16(struct Interconnect* inter, uint32_t physical_addr) {
    ...
    if (c <= 5 || (c % 100) == 0) {
        LOG_INTERCONNECT_INFO("SPU READ16 @ %08x idx=%03d count=%u -> 0x%04x", physical_addr, idx, c, val);
    }
...
void spu_write16(struct Interconnect* inter, uint32_t physical_addr, uint16_t value) {
    ...
    if (c <= 5 || (c % 100) == 0) {
        LOG_INTERCONNECT_INFO("SPU WRITE16 @ %08x idx=%03d count=%u: 0x%04x -> 0x%04x", physical_addr, idx, c, prev, value);
    }
}
