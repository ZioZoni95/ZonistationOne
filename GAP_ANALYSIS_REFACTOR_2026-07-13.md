...
**Status**: Phase 0 (cleanup) and Phase 1 (unify event scheduling) complete as of 2026-07-13, **live-tested and confirmed working** — see Changelog at bottom. Findings below are left as originally written (historical record); items resolved are marked inline with ✅.
...
**Recommended action**: Convert the GPR load-delay slot (`load_reg_idx`/`load_value` in `cpu.h`) to the same double-buffer pattern already used for GTE load delay in this same file (`gte_load_delay_*`/`gte_next_load_delay_*`) — the project already has a working, in-house reference implementation of the correct pattern one struct field away. Low-risk, mechanical change; write a small targeted test (or reuse the debug UI's Exec Trace) exercising back-to-back same-register loads before/after to confirm the fix actually changes behavior on real BIOS code, not just in theory.
...
Directly targets the still-open register/stack-corruption bug from today's live debugging (`dev_card_format → unknown(0xC8) → longjmp(garbage retval) → read(garbage args)`, see Phase 2.5). The CPU audit surfaced two concrete, actionable candidates rather than leaving this as an unguided disassembly hunt.
- [ ] **Top suspect**: convert the GPR load-delay slot to the same double-buffered pattern the project's own GTE load-delay already uses (§1.2) — real asymmetry found, plausible root cause for a back-to-back-same-register-load corruption pattern, mechanical fix using an in-house reference implementation one struct away
...
- [ ] Use the project's own Exec Trace debug-UI panel and `logs/exec_trace_systemerror_a1.log` (already capturing the tail of the exact failing sequence) for instruction-level verification once the above are in place — re-run the same region-matched-BIOS live test from Phase 2.5 as the pass/fail check
- Priority: High — directly unblocks live testing on any disc/BIOS combination, not just the one in Phase 2.5
